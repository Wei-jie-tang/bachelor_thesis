...
if (metadataFile_log.length > 2 && configFile.streaming == false) {
  ...
  ReplieServer(
    myDataActor.IP,
    configFile.localRepliePort,
    Replier_eventEmitter
  );
}

var myPublic_key = create_public_key(myDataActor.PublicKey);
var https_replier = new Replier(
  myDataActor.Port,
  Replier_eventEmitter,
  myDataActor.IP,
  myPublic_key
); // who is gonna recieve requests
var processFiles = new ProcessFiles(File, configFile.myID);
let end_event = new END_event(
  configFile.End_Event[configFile.EndRule_index],
  configFile,
  Requester_eventEmitter,
  Date.now(),
  0,
  moment().format().split("T")[0]
); // create an instant of an end event
let transaction_creator = new createTransaction(
  configFile.myID,
  privateKey,
  metadatahash,
  myPublic_key
...
let Second_Process = new SecondProcess(
  Rules_object,
  machineIDS,
  Requester_eventEmitter,
  metadatahash
...
let server_largeData = new ServerOFlargeData(
  configFile.largeDataPort,
  myDataActor.IP,
  Replier_eventEmitter
);
let judge_eventEmitter = new events.EventEmitter();
let judge_object = new Judge(
  metadatahash,
  machineIDS,
  Rules_object,
  judge_eventEmitter,
...
let external_judge = new ExternalJudge(
  configFile.externalJudgeIP,
  configFile.externalJudgePort
);
let Requesters_log = createRequestersLog(
  metadataFile_log,
  configFile.myID,
  create_public_key,
  Requester
...
let LocalActor_Connection =
  configFile.localActorType_index == 0
    ? require("./Actor_Connection/localActorTypeServer/local_connection")
    : require("./Actor_Connection/localActorTypeClient/local_connection");
let LocalActorConnection = new LocalActor_Connection(
  configFile.Actor_Connection[configFile.Connection_index],
  configFile,
  Requester_eventEmitter,
  myDataActor.IP,
  configFile.streaming
);
REPLIER(
  configFile,
  judge_object,
